...
class DeepQNetwork(object):
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            conv4 = tf.layers.conv2d(inputs=conv3_activated, filters=256,
                                     kernel_size=(3,3),strides=1, name='conv4',
                      kernel_initializer=tf.variance_scaling_initializer(scale=2))
            conv4_activated = tf.nn.relu(conv4)
            
#             conv5 = tf.layers.conv2d(inputs=conv4_activated, filters=512,
#                                      kernel_size=(3,3),strides=1, name='conv5',
#                       kernel_initializer=tf.variance_scaling_initializer(scale=2))
#             conv5_activated = tf.nn.relu(conv5)
